...
int reenumerate_PETSc(MPI_Comm *comm)
{
  ...
  int          *peer_nod_glo, *nod_glo_sizes;
  MPI_Request  *request;
  ...
  ierr = MPI_Allgather(&nmynod_glo, 1, MPI_INT, nod_glo_sizes, 1, MPI_INT, *comm);

  for(i=0;i<nproc;i++){
      if(i!=rank){
	  ierr = MPI_Isend(mynod_glo, nmynod_glo, MPI_INT, i, 0, *comm, &request[i]);
      }
  }
  for(i=0;i<nproc;i++){
      if(i!=rank){
